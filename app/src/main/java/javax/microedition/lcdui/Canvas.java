...
public abstract class Canvas extends Displayable {
	...
	public void updateSize() {
		 ...
		 */
		if (!Displayable.isFloatingMode && ContextHolder.getVk() instanceof FixedKeyboard) {
		 ...
		 */
		if (!fullscreen && !Displayable.isFloatingMode) {
		 ...
		 */
		if (!scaleToFit && !Displayable.isFloatingMode) {
			onWidth = width;
			onHeight = height;
		} else if (!keepAspectRatio || Displayable.isFloatingMode) {
			onWidth = displayWidth;
			onHeight = scaledDisplayHeight;
		} else {
			int i10 = (height * displayWidth) / width;
			onHeight = i10;
			if (i10 > scaledDisplayHeight) {
	...
	private void limitFps() {
		...
		if (isBackgroundSlot) {
			// Background slot: game loop van chay (bot + network) nhung toi uu toi da
			// 2 FPS = game loop chay 2 lan/giay, khong render (flushBuffer return early)
			// KHONG xoa cache o day — de chuyen tab muot ma, khong phai tai lai hinh
			wasHidden = true;
			if (!isAppVisible) {
				// App an + tab nen: xoa cache de giai phong RAM toi da
